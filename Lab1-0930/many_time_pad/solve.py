...
cipher_texts = [list(long_to_bytes(i)) for i in ciphers]
# print(cipher_texts)

ciphers_number = len(cipher_texts)
threshold = 8  # if bigger than this, it's a space

letters_pad = [ord(i) for i in ascii_letters] + [0]
# print(letters_pad)

def xor_bytes(a: list, b: list) -> list:
    min_len = len(a) if len(a) < len(b) else len(b)
    return [x ^ y for (x, y) in zip(a[:min_len], b[:min_len])]

def letter_positions(s: list):
    position = []
    for idx in range(len(s)):
        if s[idx] in letters_pad:
            position.append(idx)
    return position

def find_space(cipher):
    res = []
    all_positions = [0] * len(cipher)
    for another_cipher in cipher_texts:
        if another_cipher == cipher:
            continue
        xor = xor_bytes(cipher, another_cipher)
        positions = letter_positions(xor)
        for i in positions:
            all_positions[i] += 1
    for i in range(len(all_positions)):
        if all_positions[i] > threshold:
            res.append(i)
    return res

key = [0] * 256

for i in range(ciphers_number):
    positions = find_space(cipher_texts[i])
    for j in positions:
        key[j] = cipher_texts[i][j] ^ ord(' ')
...
for i in range(ciphers_number):
    msgs.append(bytes(xor_bytes(cipher_texts[i], key)))

print('key is:', bytes(key))
print('encrypted msg[11] is:', msgs[10])
